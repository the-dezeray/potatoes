...
export async function sendEnrollmentEmail(email: string, name: string, courseName: string, price: number) {
  try {
    await EmailSender.send({
      type: "courseEnrollment",
      recipients: [email],
      data: { name, courseName, price, subject: `Enrolled: ${courseName}` },
    });
    return { success: true };
  } catch (error: any) {
